...
menu: main
title: Correcting for Terminal Unicode Width
---

=====================================
Correcting for Terminal Unicode Width
=====================================

Measuring terminal Unicode width has always been a difficult problem. I present here some
very good solutions.
...
After 12 and 6 years of improving python wcwidth_ and ucs-detect_, I suggest the "Terminal Unicode
...
But grapheme support is *not* a binary indicator of terminal width measurements. With ucs-detect_, I
have measured:
...
The results of ucs-detect_ show that any specialized "with or without" grapheme width function is
...
ucs-detect_ testing of approximately 35 terminals revealed a clear pattern: most grapheme-aware
terminals supporting Mode 2027 rarely advance a single grapheme beyond 2 cells. And so the
specification_ and implementation of wcwidth 0.8.0 have been updated with the rule, "Any grapheme
...
The best general-purpose solution to the sizing problem is the `Kitty Text Sizing Protocol`_ (KTSP).
...
blessed_ library to detect and construct KTSP sequences, updating the wcwidth library function
`width()`_ to measure width of text containing KTSP sequences, so that it may be aligned with any of
the `center()`_, `ljust()`_, `rjust()`_ and high-level functions `wrap()`_ and `clip()`_.

I have also changed ucs-detect_ to report scores as '100' (perfect) for Unicode categories when KTSP is supported.
...
.. figure:: https://dxtz6bzwq9sxx.cloudfront.net/blessed_text_sizing.gif
   ...
   blessed_ demonstration script, scroller.py_ showing a classic demoscene scroller effect using the
   text sizing protocol

Python wcswidth() redefined
===========================

The Python `wcswidth()`_ function was redefined long ago, straying from POSIX:

    This implementation differs from Markus Kuhn's original POSIX C implementation, in that this
    ``wcswidth()`` processes grapheme strings yielded by `iter_graphemes()`_ defined by `Unicode
...
I decided that the existing `wcswidth()`_ interface was a natural fit to measure a series of
...
Sequence-aware width()
======================

POSIX `wcwidth(3)`_ and `wcswidth(3)`_ return ``-1`` for C0 and C1 control codes like ``TAB`` or
sequences beginning with ``ESC``.  This signals the caller to manage these "terminal functions"
themselves. In practice, many TUI applications just sum the results, discarding -1 values.

After auditing all downstream uses of Python `wcswidth()`_, `I wrote
<https://github.com/jquast/wcwidth/issues/79#issuecomment-1741605552>`_, "about every downstream
library has some issue with the POSIX wcwidth and wcswidth functions." regarding correctly
handling the -1 return values.

I made several new Python wcwidth_ releases to provide for common needs:
...
- `iter_graphemes()`_ and `iter_sequences()`_ for careful navigation of graphemes and terminal
  ...
  `iter_graphemes_reverse()`_ and `grapheme_boundary_before()`_ function for backward cursor
  control.
...
.. figure:: /images/rich-before-and-after.png
   ...
   Before and after screenshots of `rich#3956`_.
...
This is made possible by ucs-detect_, which reports all errors against the specification of wcwidth
for approximately 35 terminals; `wcstwidth()`_ makes use of this data in the form of "correction
...
There is one drawback, that we can only provide corrections for terminals that can be identified.
...
ucs-detect_ tests these multiplexers using ghostty as a "host terminal", but their displayed width
...
These correction tables create a co-dependent relationship between wcwidth_ and ucs-detect_,
increasing their maintenance burden. I hope that Unicode version releases are few, specifications
fewer, and terminal emulators accept fewer code changes for Unicode support -- so that Python
wcwidth can resume a slow release schedule even with these new corrections.

If you'd like to contribute to this or other similar works, please consider sponsering_ me on
GitHub. Your support directly enables me to maintain these projects, expand upon the greater
terminal universe, and integrate improvements directly into downstream dependencies. These
solutions can be readily ported to other languages. I'm also feeling confident enough to move more
parts of the Python wcwidth_ library to an optionally compiled C module.
